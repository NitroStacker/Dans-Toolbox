...
The detail pane opens source frames, pings object context, shows structured properties and stack frames, stores issue notes, copies full entries, and creates an evidence-bounded fix prompt only when **FIX** is pressed. Click selects one Live entry, Shift-click selects a range, Ctrl/Cmd-click toggles entries, and Ctrl/Cmd+C copies the selected entries in visible order. Visible results or an individual session can be exported as JSON or Markdown.
